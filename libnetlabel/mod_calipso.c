...
/**
 * Add a translated CALIPSO label mapping
 * @param hndl the NetLabel handle
 ...
 *
 */
int nlbl_calipso_add_trans(struct nlbl_handle *hndl,
			   nlbl_cal_doi doi,
			   struct nlbl_cal_lvl_a *lvls,
			   struct nlbl_cal_cat_a *cats)
{
	int rc = -ENOMEM;
	...
	rc = nla_put_u32(msg, NLBL_CALIPSO_A_MTYPE, CALIPSO_MAP_TRANS);
	if (rc != 0)
		goto add_std_return;
	...
	for (iter = 0; iter < lvls->size; iter++) {
		...
		rc = nla_put_u32(nest_msg_b,
				 NLBL_CALIPSO_A_MLSLVLLOC,
				 lvls->array[iter * 2]);
		if (rc != 0)
			goto add_std_return;
		rc = nla_put_u32(nest_msg_b,
				 NLBL_CALIPSO_A_MLSLVLREM,
				 lvls->array[iter * 2 + 1]);
		if (rc != 0)
			goto add_std_return;
		rc = nla_put_nested(nest_msg_a, NLBL_CALIPSO_A_MLSLVL,
				    nest_msg_b);
		if (rc != 0)
			goto add_std_return;
	...
	rc = nla_put_nested(msg, NLBL_CALIPSO_A_MLSLVLLST, nest_msg_a);
	if (rc != 0)
		goto add_std_return;
	...
	for (iter = 0; iter < cats->size; iter++) {
		...
		rc = nla_put_u32(nest_msg_b,
				 NLBL_CALIPSO_A_MLSCATLOC,
				 cats->array[iter * 2]);
		if (rc != 0)
			goto add_std_return;
		rc = nla_put_u32(nest_msg_b,
				 NLBL_CALIPSO_A_MLSCATREM,
				 cats->array[iter * 2 + 1]);
		if (rc != 0)
			goto add_std_return;
		rc = nla_put_nested(nest_msg_a, NLBL_CALIPSO_A_MLSCAT,
				    nest_msg_b);
		if (rc != 0)
			goto add_std_return;
		nlbl_msg_free(nest_msg_b);
	...
	rc = nla_put_nested(msg, NLBL_CALIPSO_A_MLSCATLST, nest_msg_a);
	if (rc != 0)
		goto add_std_return;
	nlbl_msg_free(nest_msg_a);
	nest_msg_a = NULL;
 ...
 *
 */
int nlbl_calipso_del(struct nlbl_handle *hndl, nlbl_cal_doi doi)
{
	int rc = -ENOMEM;
	struct nlbl_handle *p_hndl = hndl;
	nlbl_msg *msg = NULL;
		      ...
		      struct nlbl_cal_cat_a *cats)
{
	...
	/* sanity checks */
	if (doi == 0 ||
	    mtype == NULL || lvls == NULL || cats == NULL)
		return -EINVAL;
	if (nlbl_calipso_fid == 0)
		return -ENOPROTOOPT;
	...
	*mtype = nla_get_u32(nla_a);
	...
	if (*mtype == CALIPSO_MAP_TRANS) {
		nla_a = nlbl_attr_find(ans_msg, NLBL_CALIPSO_A_MLSLVLLST);
		if (nla_a == NULL)
			goto list_return;
		nla_for_each_attr(nla_b,
				  nla_data(nla_a), nla_len(nla_a), nla_b_rem)
		if (nla_b->nla_type == NLBL_CALIPSO_A_MLSLVL) {
		...
		nla_a = nlbl_attr_find(ans_msg, NLBL_CALIPSO_A_MLSCATLST);
		if (nla_a == NULL)
			goto list_return;
		nla_for_each_attr(nla_b,
				  nla_data(nla_a), nla_len(nla_a), nla_b_rem)
		if (nla_b->nla_type == NLBL_CALIPSO_A_MLSCAT) {
			 ...
			 nlbl_cal_mtype **mtypes)
{
	...
	int data_len;
	int data_attrlen;
	nlbl_cal_doi *doi_a = NULL, *doi_a_new;
	nlbl_cal_mtype *mtype_a = NULL, *mtype_a_new;
	uint32_t count = 0;
	...
	do {
		...
		while (nlmsg_ok(nl_hdr, data_len) &&
			...
			doi_a_new = realloc(doi_a,
					    sizeof(nlbl_cal_doi) * (count + 1));
			if (doi_a_new == NULL)
				goto listall_return;
			doi_a = doi_a_new;
			mtype_a_new = realloc(mtype_a,
					      sizeof(nlbl_cal_mtype) * (count + 1));
			if (mtype_a_new == NULL)
				goto listall_return;
			mtype_a = mtype_a_new;
